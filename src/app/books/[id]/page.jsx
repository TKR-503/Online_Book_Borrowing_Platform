  ...
  return (
    <div style={{ background: "#f0f4ff", minHeight: "100vh", padding: "28px 0" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px" }}>
        ...
        {/* Back link */}
        <Link href="/books" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "#4f46e5", textDecoration: "none", fontWeight: 600, fontSize: 14, marginBottom: 20 }}>
          <svg width="16" height="16" fill="none" stroke="#4f46e5" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
          Back to All Books
        </Link>

        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e8ecf0", overflow: "hidden", boxShadow: "0 4px 20px rgba(79,70,229,0.08)", display: "grid", gridTemplateColumns: "1fr 1.6fr" }}>

          {/* Left: Book cover */}
          <div style={{ background: "linear-gradient(135deg,#f0f4ff 0%,#e8eaf6 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 48, minHeight: 400 }}>
            <div style={{ position: "relative" }}>
              <Image src={book.image_url} alt={book.title}
                ...
                onError={e => { e.target.src = `https://placehold.co/200x270/4f46e5/fff?text=${encodeURIComponent(book.title.slice(0,8))}`; }}
              />
            </div>
          </div>
          ...
          <div style={{ padding: "36px 40px", display: "flex", flexDirection: "column" }}>
            ...
            {borrowed ? (
              ...
              <button onClick={handleBorrow} disabled={loading || book.available_quantity === 0}
                style={{ padding: "15px 24px", background: "#4f46e5", color: "#fff", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: loading ? 0.7 : 1, boxShadow: "0 4px 16px rgba(79,70,229,0.35)", transition: "opacity 0.15s" }}>
                {loading ? (
                  <><svg width="18" height="18" fill="none" viewBox="0 0 24 24" style={{ animation: "spin 0.8s linear infinite" }}><circle cx="12" cy="12" r="10" stroke="white" strokeWidth="4" opacity="0.25"/><path fill="white" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Processing...</>
                ) : (
                  <><svg width="18" height="18" fill="none" stroke="white" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>Borrow This Book</>
                )}
              </button>
            )}
          </div>
      ...
      <style>{`@keyframes spin{to{transform:rotate(360deg)}} @media(max-width:768px){.book-detail-grid{grid-template-columns:1fr!important}}`}</style>
    </div>
  );
}
